...
async function runUpdateTrunkContentStep( version, changelog, abortMessage ) {
	...
	await runStep( 'Updating trunk content', abortMessage, async () => {
		...
		console.log(
			'>> Updating the changelog in readme.txt and changelog.txt'
		);
		...
		const newChangelogContent =
			...
			changelogFileContent.substr(
				changelogFileContent.indexOf( '== Changelog ==' ) + 16
			);
...
async function runReleaseBranchCheckoutStep( abortMessage ) {
	...
	await runStep(
		...
		async () => {
			...
			const masterParsedVersion = semver.parse(
				masterPackageJson.version
			);
			releaseBranch =
				'release/' +
				masterParsedVersion.major +
				'.' +
				masterParsedVersion.minor;
...
async function runBumpPluginVersionAndCommitStep( version, abortMessage ) {
	...
	await runStep( 'Updating the plugin version', abortMessage, async () => {
		...
		console.log(
			'>> The plugin version bump has been commited successfully.'
		);
	} );
		...
		const { changelog } = await inquirer.prompt( [
			{
				...
				name: 'changelog',
				message:
					'Please provide the CHANGELOG of the release (markdown)',
			},
		] );
...
async function releasePlugin( isRC = true ) {
	...
	if ( ! isRC ) {
		...
		await askForConfirmationToContinue(
			'The Gihub release is complete. Proceed with the SVN release? ',
			abortMessage
		);
