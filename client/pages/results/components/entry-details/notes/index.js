...
import NoteIcon from '../../../../../components/icon/note-icon';

const Notes = ({ entry }) => {
	if (!entry.meta?.notes || !entry.meta?.notes?.value?.length) {
		return (
			<div className="qf-entry-notes-modal__empty">
				<NoteIcon />
				<div className="qf-entry-notes-modal__empty-text">
					{__('No Notes found', 'quillforms')}
				</div>
			</div>
		);
	}
