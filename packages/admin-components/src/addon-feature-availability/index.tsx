...
const AddonFeatureAvailability: React.FC<Props> = ({
	...
	if (isWPEnv && addon.is_installed) {
		...
		if (license?.status !== 'valid') {
			...
			licenseNote = (
				<div className="addon-feature-availability-license-note">
					Upgrading your license to {featurePlanLabel} is recommended
					to receive updates for this addon.
				</div>
			);
		}
			...
			content = (
				<div>
					...
					<div>
						...
						<NavLink
							to={`/admin.php?page=quillforms&path=addons&integration=${addonSlug}`}
						>
							addons page
						</NavLink>{' '}
						to use it.
