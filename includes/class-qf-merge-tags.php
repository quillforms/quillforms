...
class QF_Merge_Tags {
	...
	public static function process_tag( $string, $form_data, $fields, $entry_id ) {

		$merge_tag_regex = '/{{([a-zA-Z0-9]+):([a-zA-Z0-9-_]+)}}/';
		$string          = preg_replace_callback(
			$merge_tag_regex,
			function( $matches ) use ( $form_data, $fields, $entry_id ) {
				...
				$replacement         = apply_filters( 'quillforms_process_merge_tag', $default_replacement, $merge_tag_type, $merge_tag_modifier, $form_data, $fields, $entry_id );
				return $replacement;
			},
			$string
		);
	 ...
	 * @since 1.0.0
	 */
	public function process_field_merge_tag( $replacement, $merge_tag_type, $merge_tag_modifier, $form_data, $fields, $entry_id ) {
		if ( 'field' === $merge_tag_type ) {
			$field_id = $merge_tag_modifier;
			if ( ! $fields[ $field_id ] || ! $fields[ $field_id ]['blockName'] || ! $fields[ $field_id ]['value'] ) {
				return '';
			}
			$block_type = QF_Blocks_Factory::get_instance()->get_registered( $fields[ $field_id ]['blockName'] );
			return $block_type->get_merge_tag_value( $fields[ $field_id ]['value'], $form_data );
		}
		return $replacement;
	}
